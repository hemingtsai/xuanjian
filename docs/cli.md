...
```
┌──────────────────────────────────────┬──────────────────────┐
│  玄鉴 banner / 滚动输出区（流式文本、⚙工具、diff面板） │  待办清单 / 审查输出   │ ← 右侧固定面板
│  ⚙ edit  src/core/agent-loop.ts      │  ◉ 当前任务…          │
│  ┌─ diff（GitHub 风格，-红 +绿 @@行号）─┐ │  ○ 待办任务…          │
│  ⚙ write  src/new.ts                  │  ✓ 已完成…            │
│  ┌─ code（完整内容面板）──────────────┐ │                      │
├──────────────────────────────────────┼──────────────────────┤
│  ⚠ 请求权限: bash git push  [y/n/a/s] │ ← 在主输入框应答       │
│  ❯ 输入框（历史 ↑↓）                  │        [待办][审查] ← 选项卡 │
├──────────────────────────────────────┼──────────────────────┤
│ 模型:… agent:… 模式:… 工作区:… LSP:… DAP:… ctx:… │ Ctrl-T 切换 │
└──────────────────────────────────────┴──────────────────────┘
```
...
**右侧面板**：屏幕右侧固定宽度面板，实时展示待办清单与审查流水线输出。底部选项卡切换（`Ctrl-T`），底部右下角显示当前激活的选项卡。

**输入即待办**：用户每提交一条信息，都会先被创建为一个待办（置为"当前进行中"），agent 回合结束后自动标记完成；agent 的 `todowrite` 可接管后续的待办拆分与进度。

**审查自动修正**：运行 `/review` 或 goal 里程碑审查后，若发现问题（critical/warning），修正任务会自动插入到**当前正在执行 todo 的下一个**（goal 模式会写入任务 DAG，当前任务完成后自动执行修正）。

**Provider 连接引导**：未配置模型且无任何 provider 凭据时，输入框上方显示 `尚未连接任何 provider` 引导条。按 `C-o` 或输入 `/auth` 打开连接向导（provider 选择 → API key 输入 → 保存并设默认模型）。

**主输入框应答**：权限确认、提问、provider 选择都不再用弹窗，而是输出到滚动区提示，在主输入框直接输入即可应答（权限：`y` 允许 / `n` 拒绝 / `a` 本次会话 / `s` 总是允许）。
